...
import { Link2, ArrowRight, ShieldCheck, ShieldX, ExternalLink } from 'lucide-react';
...
export default async function SSOSettingsPage() {
  ...
  const configuredCallbacks = apps.filter((a) => (a.allowed_callback_urls ?? []).length > 0).length;
      ...
      <main className="container mx-auto space-y-8 p-4 py-8">
        <div className="flex items-start justify-between gap-4">
          <div className="space-y-2">
            ...
            <p className="text-muted-foreground max-w-2xl">
              Manage SSO integration settings across apps. Callback URLs prevent open redirects; app secrets can secure
              code exchange.
            </p>
          </div>
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">Configured applications</p>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">Apps with allowlisted callbacks</p>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">Apps with an exchange secret configured</p>
            </CardContent>
          </Card>
        </div>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between">
            <div className="space-y-1">
              ...
              <p className="text-sm text-muted-foreground">Open an app’s SSO tab to edit settings.</p>
            </div>
          </CardHeader>
                    ...
                    >
                      <div className="min-w-0 space-y-1">
                        <div className="flex items-center gap-2">
                          ...
                          <code className="rounded bg-muted px-2 py-0.5 text-xs font-mono">{app.id}</code>
                          {app.enabled ? (
                        ...
                        <div className="flex flex-wrap items-center gap-2 text-sm text-muted-foreground">
                          <span className="flex items-center gap-1">
                            ...
                            {callbackCount} callback URL{callbackCount === 1 ? '' : 's'}
                          </span>
        ...
        <Card className="border-primary/20 bg-primary/5">
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2">
              <label className="text-sm font-medium text-muted-foreground">Portal Base URL</label>
              <div className="flex items-center gap-2">
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-muted-foreground">SSO Login Endpoint</label>
              <div className="flex items-center gap-2">
                <code className="flex-1 rounded-md bg-muted px-3 py-2 font-mono text-sm break-all">
                  {portalUrl}/login?app_id=YOUR_APP_ID&redirect_uri=YOUR_CALLBACK
                </code>
                <CopyUrlButton url={`${portalUrl}/login?app_id=YOUR_APP_ID&redirect_uri=YOUR_CALLBACK`} label="Copy" />
              </div>
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium text-muted-foreground">Code Exchange Endpoint</label>
              <div className="flex items-center gap-2">
                <code className="flex-1 rounded-md bg-muted px-3 py-2 font-mono text-sm break-all">
            ...
            <p className="text-xs text-muted-foreground">
              See the <Link href="/docs/sso-integration-guide" className="text-primary underline underline-offset-2 hover:no-underline">SSO Integration Guide</Link> for implementation details.
            </p>
          </CardContent>
        </Card>
      </main>
